const BirdBlotter_contract = artifacts.require("./BirdBlotter.sol");
const MoonbirdTest_contract = artifacts.require("./MoonbirdTest.sol");
...
contract("BB", accounts => {
  ...
  beforeEach(async() =>{
    BirdBlotter = await BirdBlotter_contract.deployed();
    MoonbirdTest = await MoonbirdTest_contract.deployed();
    await web3.eth.accounts.wallet.create(1)
  });
  
  it("... should deploy with less than 4.7 mil gas", async () => {
    let BirdBlotterInstance = await BirdBlotter_contract.new();
    let receipt = await web3.eth.getTransactionReceipt(BirdBlotterInstance.transactionHash);
